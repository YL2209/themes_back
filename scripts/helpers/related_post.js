...
hexo.extend.helper.register('related_posts', function (currentPost, allPosts) {
  ...
  if (relatedPosts.length > 0) {
    result += '<input class="card-widget-visible" id="card-recommend-post-anchor" type="radio" name="card-widget-visible" value="card-recommend-post"/>'
    ...
    for (let i = 0; i < Math.min(relatedPosts.length, limitNum); i++) {
      const cover =
        relatedPosts[i].cover === false
          ...
          : relatedPosts[i].cover     //https://akilar.top/posts/194e1534/ Butterfly布局调整———相关推荐版块侧栏卡片化
      const title = this.escape_html(relatedPosts[i].title)
      result += `<div class="aside-list-item">`
      result += `<a class="thumbnail" href="${this.url_for(relatedPosts[i].path)}" title="${title}"><img src="${this.url_for(cover)}" alt="${title}"></a>`
